...
link-citations: yes
description: "This is how you build a Buggy Bar."
---
...
```{r complete-bar, fig.cap="The finished product, bundled and ready for installation on a kite", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/finished_bar.jpg")
```
...
* 1 [Separation Block v2 6mm trimline bore release 1.4.2](https://github.com/pbchase/kite_bar_parts/blob/master/printable/separation_block_v2_6mm_trimline_bore_7e61bdd.stl)
* 1 [Cleat Bead release 1.4.2](https://github.com/pbchase/kite_bar_parts/blob/1.4.2/printable/cleat_bead_ff7e41a.stl)
* 1 [Chickenloop Handle](https://github.com/pbchase/kite_bar_parts/blob/master/printable/chickenloop_release_d1928a2.stl)
...
```{r bare-bar, fig.cap="", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/bare-bars.jpg")
```
...
```{r kite-side-borehole, fig.cap="Kite-side of the bar.", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/kite-side-borehole.jpg")
```
...
```{r pilot-side-borehole, fig.cap="Pilot-side of the bar.", echo = FALSE, out.width='50%', fig.align='center'}
knitr::include_graphics("images/pilot-side-borehole.jpg")
```


## Trim Line

To build the trimline, you will need 2510mm of Amsteel Blue (use only the gray color), 80mm x 105m of insignia cloth (dark colors are better), two 18mm x 3mm nickel-plated neodymium disc magnets, one small stopper ball and one medium stopper ball. 

Cut a segment of gray 4mm Amsteel Blue to a length of 2510mm with a fresh razor blade to get a clean cut. 

### Threading end of the trim line

One end of the trim line will be wrapped in a strip of insignia cloth that extends off the end to form a strip of fabric used to thread the trim line through the other components. Cut a strip of insignia cloth 20mm x 90mm to use as this wrap on the threading end.
...
```{r thread-end, fig.cap="Threading end of the trim line formed with a wrap of insignia cloth", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/thread_end_of_trimline.jpg")
```


### Loose end of the trim line
...
```{r finished-end, fig.cap="Grip end of the trim line with balls offset to show knots (above) and thread end (below)", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/finished_end_of_trimline.jpg")
```

Form the wrap for the magents by cutting a strip of sailmaker's insignia cloth according to the dimensions shown in Figure \@ref(fig:magnet-wrap) below to wrap the magnets.

```{r magnet-wrap, fig.cap="Insignia cloth patch to hold trimline magnets", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/magnetic_trimline_tails_9.jpg")
```

To make the reinforcement for the wrap, cut a segment of round, 0.95" diameter plastic weed-whacker cord 150mm long to wrap around the magnets and up the trim line. Bend the plastic cord until the ends meet, then clamp the ends together with a spring clip. Heat the middle of the plastic cord with a low flame. Dance the cord through the flame flipping and turning as you go to assure event heating. When the bend relaxes, form it around a 16mm x 36mm mandrel of CPVC pipe. The mandrel can be made from 5/8" diameter CPVC piping, a few strip cut from a plastic bucket to form a spacer and two screws to hold the parts together. The assembled mandrel is shown in \@ref(fig:plastic-cord-mandrel) and \@ref(fig:plastic-cord-mandrel-end-view)

```{r plastic-cord-mandrel, fig.cap="Mandrel for forming the plastic cord that surrounds the magnets", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/plastic_cord_mandrel.jpg")
```

```{r plastic-cord-mandrel-end-view, fig.cap="End view of mandrel", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/plastic_cord_mandrel_end_view.jpg")
```
...
Thread a medium stopper ball onto the trimline, recessed side first. Slide it all the way to the knot next to the magnet wrap. Mark the trimline 65mm above the top of the medium ball. Tie an overhand knot in the trimline just above that mark. Now thread a small ball onto the trim line and slide it all the way to the knot. The finished product can be seen in Figure \@ref(fig:finished-end)


## Cleat Jacket

The Aero Cleat requires a jacket of ferromagnetic stainless steel sheet to create a tether point for the trimline tail. Make a template from the image in Figure \@ref(fig:cleat-jacket-template) or reference the [original design](https://github.com/pbchase/kite_bar_parts/blob/master/cleat_jacket_template.svg). Trace the pattern onto a sheet of 0.025" thick 430 stainless steel sheet. Only use 430 stainless steel for the cleat jacket; other grades of stainless steel will not attract the magnets in the trimline. Cut the shape from the metal sheet, drill out the two holes with a 5/32" bit and ease all the edges with a metal file. Most of the edges will be on grippable surfaces, so do a good job of easing them.
...
```{r cleat-jacket-template, fig.cap="Template for making an Aero Cleat Jacket", echo = FALSE, out.width='100%', fig.align='center'}
knitr::include_graphics("images/cleat_jacket_template.pdf")
```


## Separation block

The juncture of the flying lines and the trim line is managed by the _separation block_ as shown in Figure \@ref(fig:separation-block). 

```{r separation-block, fig.cap="Picture of separation block", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/separation_block.jpg")
```

This component provides a curved path to serve as a pully for the trim line and a guide path for each of the main flying lines. The [3-D model](https://github.com/pbchase/kite_bar_parts/blob/master/printable/separation_block_v2_6mm_trimline_bore_7e61bdd.stl) for this component is accessible in the [Kite Bar Barts](https://github.com/pbchase/kite_bar_parts) repository. Should you print this model, use a 1/8" drill to clean out the flying-line bore holes before installation. This step will simplify threading the flying lines.


## Flag line

The _flag line_ allows the kite to be flagged out via one of the top lines. It bridges the distance from the flying line at the underside of the separation block and the underside of the kite bar. The line is about 1400 of 1/16" Ultrex 12 that encloses about 400mm of 2mm bungie. A short loop is incorporated into each end. This design allows the flag line to stay taught whether the bar is trimmed in or out. The Ultrex jacket can handle the loads experienced during release to assure a clean flag-out without damage to the flag line. The dimensions for the flag line are tightly constrained by the need to maintain line tension throughout the trim range. They can be calculated using a copy of the Google Sheet [kite bar elastic flag line length calculations](http://tinyurl.com/y4k5chgv) ([http://tinyurl.com/y4k5chgv](http://tinyurl.com/y4k5chgv))

### Forming the ends

To build the flag line, cut 1370mm of 1/16" Ultrex 12. Mark each end at 45mm, 65mm, 85mm, and 140mm. The 65mm will be the end of the loop while the 45mm and 85mm marks will align at the bottom of the loop. Make a brummel splice at one end to secure that loop. 

The opposite end of the flag line is more complex owing to its need to secure the flying line against the separation block. The 1/16" Ultrex is not large enough to not pull through the separation block under load. To prevent that, we'll bury a segment of Ultrex inside the loop at this end of the flag line to make it fatter. Start by form the first loop of the brummel splice. Then splicing a scrap of 1/16" Ultrex about 40mm long into the flag line between the 45mm and 85mm marks. Complete the brummel splice, and cut the ends off the short added segment and bury the remainder. 

### Inserting the bungie

Unspool a meter of 2mm bungie. You can leave it attached to spool for this step. Add a threading end of insignia cloth to the free end of the bungie. A rectangle of peel-and-steek insignia cloth about 8mm x 40mm will work well. Measure 390mm from the back-end of the thread end and mark this point with a narrow strip of insignia cloth. Route a very long fid inside the flag line from the near end of one loop to the near end of the other. The fid should occupy the entire length of the otherwise unoccupied flag line. Pull the 2mm bungie through the jacket and out the opposite end until only the thread end cloth is visible. Clamp this end with a spring clip. Align the strip of insignia cloth with the other entry-point into the flag line jacket. Clamp this end with another other spring clip.

Use a narrow, 2m long stick and some cord to stretch the flag line. Anchor the flag line at one end of the stick. Use cord affixed to the opposite end of the stick to put the flag line under tension. Start milking the jacketof the flag lineback and forth to evenly distribute the jacket and bungie. Retension the flag line and milk it again. Continuing milking the jacket back and forth and retensioning until it stops elongating. With the flag line still stretched and the bungie still constrained by spring clips, sew the length of the flag line from spliced loop to spliced loop, back stitching at each end. Use a straight stitch for this step.
...
```{r flagline-disconnect, fig.cap="Flagline disconnect", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/flagline_disconnect.jpg")
```


## Stub line

The non-flagging flying line needs to be terminated at the underside of the separation block in the same manner as the flying line that _does_ flag. This is accomplished with a short loop called the _stub line_. To match the upper end of the flag line, the stub line is made from 300mm of 1/16" Ultrex 12.
...
```{r steering-lines, fig.cap="Steering line construction details", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/04_kite_bar_rigging_2015-10-06_2.png")
```

## Mini-chickenloop

This bar uses a small chickenloop that provides a quick disconnect from the kite, trim line rention, and a below-the-bar grip. The quick disconnect will separate the trim line from the flier's harness when pushed away. The trim lime retention feature is required to keep the bar from sliding off the end of the trim line. Below-the-bar grip is useful for Arc fliers when disconnecting while the kite is at zenith. The chickenloop is as short as possible to maximize the flier's reach and useable bar throw. That's how it gets the moniker _mini-chickenloop_ or _MCL_.

```{r mini-chickenloop, fig.cap="Assembled mini-chickenloop ", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/mini_chickenloop.jpg")
```

The MCL is constructed from 320mm of 2.5mm (7/64") Amsteel Blue, 140mm of 2mm bungie, a 50mm cotter pin, a 24mm pin of 0.102" diameter stainless steel, and a 3-D printed chickenloop release. The 3-D model for the [chickenloop release](https://github.com/pbchase/kite_bar_parts/blob/master/printable/chickenloop_release_d1928a2.stl) is accessible in the [Kite Bar Barts](https://github.com/pbchase/kite_bar_parts) repository. Examples of the printed part are shown in Figure \@ref(fig:chickenloop-release).

```{r chickenloop-release, fig.cap="Mini-chickenloops as printed", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/chickenloop_release.jpg")
```

### Preparing the chickenloop release

Drill our the long bores of the chickenloop relase with a 7/64" drill. Drill it out in two passes the avoid stressing the thin walls of these bores holes. For the cross bore, again use a 7/64" drill, but do not drill completely through both walls of the chickenloop release. Instead, drill completely throuhgh the first wall, across the part, and partially into the second wall. This will make the initial insertion and alignment of the trimline retention pin easy.

Cut a 24mm segment of the 0.102" diameter stainless rod. Ease each end of this pin with a metal file. Hold the pin in pliers to briefly heat the one tip of the pin in a flame. Two seconds in the heat of a propane torch is about enough. You want it hot enough to soften the ABS plastic of the chickenloop release, but no hotter. Slide the hot end of the pin into the completely drilled hole until it stops. Press the pin fully in with a mallet or pliers.

```{r chickenloop-release-with-pin, fig.cap="Mini-chickenloop with the trimline retention pin installed", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/chickenloop_release_with_pin.jpg")
```

### Preparing the chickenloop line

Cut the 2.5mm Amsteel 320mm long. Measure from each end to make a mark aty 52mm and 95mm. Also mark the midline 160mm from either end. Make loops at each end using brummel splices. At each end, pierce the line at the first mark, to pull the send mark through the hole. When burying the ends, each should terminate very close the midline.

With the line complete, cut a 50mm cotter pin to 40mm in length and ease the cut end with a file. Gently spread the pin, slide one of the Amsteel loops in the gap, and force it down to the eye of the cotter pin.

### Preparing the bungie

Cut 140mm of 2mm bungie. Apply a little bit of cyanoacrylate glue on each end and cure it with activater. Use a razor blade to trim the flared part of the end off. Tie an overhand knot at the very tip of one end.

### Assembling the MCL

Thead the untied end of the bungie from the top of the chickenloop release and out the bottom. Thread the bungie through the eye of the cotter pin then up the through the remaining vertical bore hole and out the top. Tie an overhand knot in the end of the bungie to secure the end at the top of the chickenloop release. 
...
To thread the trim line, put the threaded end through the cleat, entering at the jam side. Exit the cleat at the top, routing up through the low friction ring of the separation block. Reenter the cleat and go through its serpentine path. Leave just enough line above the cleat to provide the desired trim. Route the line through the cleat bead. Go down through the center of the bar, through a ~25mm ball to retain the trim line in the bar.  Then go back through the center bore of the bar and through the cleat bead. Slide the cleat beaddown towards the bar to leave lots of slack next to the cleat. Before going any further, make sure the line already passing through the cleat's serpentine has 5 cm of loop on each side of the cleat. You'll need this slack in the next step.
...
```{r thread-cleat-1, fig.cap="Form large parallel loops of line on each side of the cleat's serpentine path. Route about 5cm of the trim out of the top of the cleat.", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/threading_the_cleat_1.jpg")
```



```{r thread-cleat-2, fig.cap="Route the 5cm of trim line tail through the loops along the side of the serpentine.", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/threading_the_cleat_2.jpg")
```

```{r thread-cleat-3, fig.cap="Route the insignia cloth tail through the loops. Holding the tail in place, pull the trim line slack down through the cleat grooming the lines as you go. Keep the lines as parallel as possible. Take the slack out of the trimline loops 1 of 3.", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/threading_the_cleat_3.jpg")
```

```{r thread-cleat-4, fig.cap="Take the slack out of the trimline loops 2 of 3.", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/threading_the_cleat_4.jpg")
```

```{r thread-cleat-5, fig.cap="Take the slack out of the trimline loops 3 of 3.", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/threading_the_cleat_5.jpg")
```
...
```{r flag-line-at-separation-block, fig.cap="The flag line meets the flying line.", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/flag-line-at-separation-block.jpg")
```

```{r upper-section-of-assembled-bar, fig.cap="Assembled bar", echo = FALSE, out.width='70%', fig.align='center'}
knitr::include_graphics("images/upper_section_of_assembled_bar.jpg")
```
